...
You can enable [automatic account creation](/installation/configuration.mdx#komga_oauth2accountcreation--komgaoauth2-account-creation-truefalse) upon successful social login.
...
If you encounter issues, you may need to enable `DEBUG` logs.
...
<Tabs>
    ...
    ```shell
    java -jar komga-x.y.z.jar --logging.level.org.springframework.security=DEBUG
    ```
    </TabItem>
...
#### Incompatible JWT signing algorithm

```log title="komga.log"
Authentication failed with provider OidcAuthorizationCodeAuthenticationProvider since [invalid_id_token] An error occurred while attempting to decode the Jwt: Signed JWT rejected: Another algorithm expected, or no matching key(s) found
```

Komga expects `RS256` algorithm to verify the signature of the JWT.

You can check the configured algorithm in the `.well-known` endpoint of your provider, under the `id_token_signing_alg_values_supported` section, and ensure it is set to `RS256`.